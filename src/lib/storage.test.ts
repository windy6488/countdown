...
import { EVENTS_STORAGE_KEY, readEvents, writeEvents, type StorageLike } from './storage';
...
describe('storage 读写与损坏回退', () => {
  ...
  it('含非法条目时只保留合法条目并标记 ok=false', () => {
    const storage = memoryStorage({
      [EVENTS_STORAGE_KEY]: JSON.stringify([ev, { id: 'bad', name: '' }])
    });
    const res = readEvents(storage);
    expect(res.events).toEqual([ev]);
    expect(res.ok).toBe(false);
  });
